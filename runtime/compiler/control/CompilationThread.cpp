...
TR_YesNoMaybe TR::CompilationInfo::shouldActivateNewCompThread()
{
   ...
   if (getNumCompThreadsActive() > 0 && javaVM->internalVMFunctions->isDebugOnRestoreEnabled(javaVM) && javaVM->internalVMFunctions->isCheckpointAllowed(javaVM))
   {
      return TR_no;
   }
...
bool TR::CompilationInfo::isMethodIneligibleForAot(J9Method *method)
{
...
bool TR::CompilationInfoPerThreadBase::compilationThreadIsActive()
{
   bool compThreadIsActive =
       _compilationThreadState == COMPTHREAD_ACTIVE || _compilationThreadState == COMPTHREAD_SIGNAL_WAIT || _compilationThreadState == COMPTHREAD_WAITING;
...
TR::CompilationInfoPerThreadBase::CompilationInfoPerThreadBase(TR::CompilationInfo &compInfo, J9JITConfig *jitConfig, int32_t id, bool onSeparateThread) : _compInfo(compInfo),
...
void TR::CompilationInfo::initCPUEntitlement()
{
...
bool TR::CompilationInfo::shouldRetryCompilation(J9VMThread *vmThread, TR_MethodToBeCompiled *entry, TR::Compilation *comp)
{
   ...
   else if (entry->_compErrCode != compilationOK)
   {
      ...
      if (javaVM->internalVMFunctions->isDebugOnRestoreEnabled(javaVM) && javaVM->internalVMFunctions->isCheckpointAllowed(javaVM) && !compInfo->getCRRuntime()->isCheckpointInProgress() && !compInfo->isInShutdownMode())
      {
...
TR::CompilationInfo::startCompilationThread(int32_t priority, int32_t threadId, bool isDiagnosticThread)
{
   ...
   if (!isDiagnosticThread)
   {
      if ((_numCompThreads > TR::Options::_numUsableCompilationThreads) || (_numAllocatedCompThreads >= TR::Options::_numAllocatedCompilationThreads))
      {
         return 1;
      }
   }
...
#if defined(J9VM_OPT_JITSERVER)
   TR::CompilationInfoPerThread *compInfoPT = getPersistentInfo()->getRemoteCompilationMode() == JITServer::SERVER ? new (persistentMemory()) TR::CompilationInfoPerThreadRemote(*this, _jitConfig, threadId, isDiagnosticThread) : new (persistentMemory()) TR::CompilationInfoPerThread(*this, _jitConfig, threadId, isDiagnosticThread);
#else
...
void TR::CompilationInfo::cleanDLTRecordOnUnload()
{
   for (int32_t i = 0; i < DLT_HASHSIZE; i++)
   {
      ...
      while (curr != NULL)
      {
         ...
         if (J9_ARE_ALL_BITS_SET(clazz->classLoader->gcFlags, J9_GC_CLASS_LOADER_DEAD) || (J9CLASS_FLAGS(clazz) & J9AccClassDying))
         {
            if (prev == NULL)
               _dltHash[i] = next;
            else
...
void TR::CompilationInfoPerThread::processEntry(TR_MethodToBeCompiled &entry, J9::J9SegmentProvider &scratchSegmentProvider)
{
   TR::CompilationInfo *compInfo = getCompilationInfo();
   J9VMThread *compThread = getCompilationThread();
   TR::IlGeneratorMethodDetails &details = entry.getMethodDetails();
   ...
   if (
       ...
       && (compInfo->getRampDownMCT()             // force to have only one thread active
           || compInfo->getSuspendThreadDueToLowPhysicalMemory() || (!tryCompilingAgain
                                                                     /*&& compInfoPT->getCompThreadId() != compInfo->getFirstCompThreadID()*/
                                                                     && TR::Options::getCmdLineOptions()->getOption(TR_SuspendEarly) && compInfo->getQueueWeight() < TR::CompilationInfo::getCompThreadSuspensionThreshold(compInfo->getNumCompThreadsActive()))
#if defined(J9VM_OPT_JITSERVER)
           || (compInfo->getPersistentInfo()->getRemoteCompilationMode() == JITServer::CLIENT && compInfo->getCompThreadActivationPolicy() == JITServer::CompThreadActivationPolicy::SUSPEND) // keep suspending threads until server space frees up
#endif
           ))
   {
      // Suspend this thread
...
bool TR::CompilationInfoPerThread::shouldPerformCompilation(TR_MethodToBeCompiled &entry)
{
   ...
   if ((TR::Options::getCmdLineOptions()->getOption(TR_EnableHCR) || TR::Options::getCmdLineOptions()->getOption(TR_FullSpeedDebug)) && details.getClass() && J9_IS_CLASS_OBSOLETE(details.getClass()))
      return false;
            ...
            if (!nextMethodToBeCompiled)
            {
               *compThreadAction = GO_TO_SLEEP_CONCURRENT_EXPENSIVE_REQUESTS;
               ...
               for (int32_t i = getFirstCompThreadID(); i <= getLastCompThreadID(); i++)
               {
                  ...
                  CompilationThreadState currentThreadState = curCompThreadInfoPT->getCompilationThreadState();
                  if (
                      currentThreadState == COMPTHREAD_ACTIVE || currentThreadState == COMPTHREAD_SIGNAL_WAIT || currentThreadState == COMPTHREAD_WAITING || currentThreadState == COMPTHREAD_SIGNAL_SUSPEND)
                  {
                     if (curCompThreadInfoPT->getMethodBeingCompiled() &&
...
#endif
        || getPersistentInfo()->getDisableFurtherCompilation() || (_jitConfig->runtimeFlags & (J9JIT_CODE_CACHE_FULL | J9JIT_DATA_CACHE_FULL))) &&
...
#ifdef TR_HOST_S390
void TR::CompilationInfoPerThreadBase::outputVerboseMMapEntry(
...
void TR::CompilationInfoPerThreadBase::outputVerboseMMapEntries(
    TR_ResolvedMethod *compilee,
 ...
 */
void TR::CompilationInfoPerThreadBase::preCompilationTasks(J9VMThread *vmThread,
                                                           TR_MethodToBeCompiled *entry,
                                                           J9Method *method,
                                                           const void **aotCachedMethod,
                                                           TR_Memory &trMemory,
                                                           ...
                                                           TR_RelocationRuntime *reloRuntime)
{
   ...
   if (entry->_methodIsInSharedCache == TR_yes // possible AOT load
       && !TR::CompilationInfo::isCompiled(method) && !entry->_doNotAOTCompile && !TR::Options::getAOTCmdLineOptions()->getOption(TR_NoLoadAOT) && !(_jitConfig->runtimeFlags & J9JIT_TOSS_CODE) && !_jitConfig->inlineFieldWatches)
   {
   ...
   if (!entry->isAotLoad()) // We don't/can't relocate this method
   {
      ...
      else
      {
         ...
         bool withoutSCCEligibleForRelocatableCompile =
             ...
             !entry->isJNINative() && !details.isNewInstanceThunk() && !details.isMethodHandleThunk() && !entry->isDLTCompile()
             ...
             // Eligibility checks
             && !_compInfo.isMethodIneligibleForAot(method) && (!TR::Options::getAOTCmdLineOptions()->getOption(TR_AOTCompileOnlyFromBootstrap) || fe->isClassLibraryMethod((TR_OpaqueMethodBlock *)method), true)
         ...
         eligibleForRemoteAOTNoSCCCompile =
             withoutSCCEligibleForRelocatableCompile && persistentInfo->getRemoteCompilationMode() == JITServer::CLIENT && persistentInfo->getJITServerUseAOTCache() // Ideally we would check if the options allow us to use the AOT cache for this method
             && persistentInfo->getJITServerAOTCacheIgnoreLocalSCC()                                                                                                 // Need to be using the new implementation
             && !entry->_doNotLoadFromJITServerAOTCache && !persistentInfo->doNotRequestJITServerAOTCacheStore() && !_compInfo.getLowCompDensityMode()               // AOT req is sent to JITServer and we don't want to send JITServer any messages in this mode
      ...
      if (entry->isAotLoad() && entry->_oldStartPC == 0 && startPC != 0) // AOT load that succeeded
      {
             ...
             (TR::Options::getAOTCmdLineOptions()->getEnableSCHintFlags() & (TR_HintUpgrade | TR_HintHot | TR_HintScorching)))
         {
            ...
            if (hints)
            {
               ...
               if (TR::Options::getAOTCmdLineOptions()->getOption(TR_EnableSymbolValidationManager) && !disableQueueLPQAOTUpgrade)
               {
...
#if defined(J9VM_OPT_JITSERVER)
   if (_compiler && _compiler->getPersistentInfo()->getRemoteCompilationMode() == JITServer::SERVER && !entry->_optimizationPlan->isLogCompilation())
   {
                                          ...
                                          J9::J9SegmentProvider &scratchSegmentProvider)
{
   ...
   try
   {
      ...
      TR::SegmentAllocator &regionSegmentProvider =
          TR::Options::getCmdLineOptions()->getOption(TR_EnableScratchMemoryDebugging) ? static_cast<TR::SegmentAllocator &>(debugSegmentProvider) : static_cast<TR::SegmentAllocator &>(defaultSegmentProvider);
      ...
      if (
          (TR::Options::canJITCompile() || canDoRelocatableCompile || entry->isAotLoad() || entry->isRemoteCompReq())
...
#endif /* defined(J9VM_OPT_JITSERVER) */
         {
            ...
            if (!that->_methodBeingCompiled->isAotLoad()) // exclude AOT loads
            {
                   ...
                   that->getCompilationInfo()->getPersistentInfo()->getJitState() == IDLE_STATE)
               {
                  ...
                  if (jitConfig->javaVM->phase != J9VM_PHASE_NOT_STARTUP && (disableNextGenHCRDuringStartup || that->_methodBeingCompiled->isDLTCompile() || (options->getOptLevel() <= warm && !enableStartupNextGenHCRAtAllOpts)))
                  {
                     options->setOption(TR_DisableNextGenHCR);
                  }
    ...
    TR::SegmentAllocator const &scratchSegmentProvider)
{
   ...
   try
   {
      ...
      if (
          _methodBeingCompiled->_oldStartPC && compiler->getOption(TR_FailPreXRecompile) && TR::Recompilation::getJittedBodyInfoFromPC(_methodBeingCompiled->_oldStartPC)->getIsInvalidated())
      {
      ...
      )
      {
         ...
         {
         public:
            TraceMethodMetadata(TR::Compilation &compiler) : _compiler(compiler),
         ...
         if (!(jitConfig->runtimeFlags & J9JIT_TOSS_CODE))
         {
            ...
            else // Non-AOT compilation or remote JIT/AOT compilations (at the JITClient)
            {
               ...
               else
               {
                  jitMethodTranslated(vmThread, method, startPC);
#if defined(J9VM_OPT_CRIU_SUPPORT)
                  if (jitConfig->javaVM->internalVMFunctions->isCheckpointAllowed(jitConfig->javaVM) && jitConfig->javaVM->internalVMFunctions->isDebugOnRestoreEnabled(jitConfig->javaVM) && (!compInfo->getCRRuntime()->isCheckpointInProgress() || comp->getOption(TR_FullSpeedDebug)))
                  {
...
//
bool TR::CompilationInfoPerThreadBase::methodCanBeCompiled(TR_Memory *trMemory, TR_FrontEnd *vm, TR_ResolvedMethod *method, TR_FilterBST *&filter)
{
   filter = NULL;
    ...
    TR_OptimizationPlan *optimizationPlan)
{
   if (!_methodBeingCompiled->isAotLoad())
   {
      ...
      else
      {
         ...
         if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompileEnd, TR_VerboseGc, TR_VerboseRecompile, TR_VerbosePerformance, TR_VerboseOptimizer) || (compiler->getOption(TR_CountOptTransformations) && compiler->getVerboseOptTransformationCount() >= 1))
         {
...
void TR::CompilationInfoPerThreadBase::processException(
    J9VMThread *vmThread,
    TR::SegmentAllocator const &scratchSegmentProvider,
    TR::Compilation *compiler,
    volatile bool &haveLockedClassUnloadMonitor,
    const char *exceptionName) throw()
{
   if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch))
   {
      TR_VerboseLog::writeLineLocked(
...
void TR::CompilationInfoPerThreadBase::processExceptionCommonTasks(
    J9VMThread *vmThread,
    TR::SegmentAllocator const &scratchSegmentProvider,
    TR::Compilation *compiler,
    const char *exceptionName,
...
void TR::CompilationInfo::debugPrint(const char *debugString, TR::IlGeneratorMethodDetails &details, J9VMThread *vmThread)
{
   if (!_traceCompiling)
      return;
   fprintf(stderr, "%8" OMR_PRIxPTR ":", (uintptr_t)vmThread);
...
TR::CompilationInfo::computeFreePhysicalMemory(bool &incompleteInfo)
{
   ...
   J9MemoryInfo memInfo;
   if (0 == j9sysinfo_get_memory_info(&memInfo) && memInfo.availPhysical != OMRPORT_MEMINFO_NOT_AVAILABLE && memInfo.hostAvailPhysical != OMRPORT_MEMINFO_NOT_AVAILABLE)
   {
...
void TR::CompilationInfo::addJ9HookVMDynamicCodeLoadForAOT(J9VMThread *vmThread, J9Method *method, J9JITConfig *jitConfig, TR_MethodMetaData *relocatedMetaData)
{
   OMR::CodeCacheMethodHeader *ccMethodHeader;
...
#if defined(J9VM_INTERP_AOT_COMPILE_SUPPORT) && defined(J9VM_OPT_SHARED_CLASSES) && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) || defined(TR_HOST_ARM64))
void TR::CompilationInfo::storeAOTInSharedCache(
    J9VMThread *vmThread,
         ...
         if (!entry->_queuedForCompilation)
         {
            ...
            else
            {
               ...
               if (isLoopy)
               {
                  ...
                  int32_t guessedInitialCount = TR::Options::getCountsAreProvidedByUser() ? TR::Options::getCmdLineOptions()->getInitialBCount() : TR_DEFAULT_INITIAL_BCOUNT;
